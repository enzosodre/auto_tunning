...
def setup_parameters():
    ...
    num_params = get_user_input(
        "Quantos parâmetros o executável recebe? ", int, lambda v: v > 0
    )
    ...
    for i in range(num_params):
        ...
        p_type = get_user_input(
            p_type_prompt, str, lambda v: v.lower() in ["cat", "int"]
        )

        if p_type.lower() == "cat":
            # 3b. Categórico
            options_prompt = (
                "Digite as opções separadas por vírgula (ex: baixo,medio,alto): "
            )
            options_str = get_user_input(options_prompt, str, lambda v: len(v) > 0)
            options_list = [opt.strip() for opt in options_str.split(",")]

            param_definitions.append({"type": "cat", "options": options_list})

        elif p_type.lower() == "int":
            # 3c. Inteiro
            p_min = get_user_input(f"Valor MÍNIMO para o parâmetro {i+1}: ", int)
            p_max = get_user_input(
                f"Valor MÁXIMO para o parâmetro {i+1}: ",
                int,
                lambda v: v >= p_min,
            )
...
# =============================================================================
# ### FUNÇÃO 4: O ALGORITMO (PATTERN SEARCH) ###
# =============================================================================
def run_pattern_search(
    start_individual, param_definitions, end_time, objective_multiplier, results_queue
):
    """
    ...
    int_ranges = [
        p["max"] - p["min"] for p in param_definitions if p["type"] == "int"
    ]
            ...
            for i in range(len(param_definitions)):
                ...
                if p_def["type"] == "cat":
                    ...
                    for direction in [1, -1]:
                        ...
                        new_val = max(
                            p_def["min"], min(p_def["max"], int(new_val))
                        )
                ...
                if best_fitness_in_axis > current_best_fitness:
                    ...
                    results_queue.put(
                        (current_best_fitness, current_best_individual)
                    )
                    improved_in_this_step = True
...
def main():
    ...
    print(
        f"Início: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start_time))}"
    )
    print(
        f"Término previsto: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(end_time))}"
    )
    print("=" * 60)
    ...
    print(
        f"Iniciando {WORKER_COUNT} buscas paralelas com os seguintes pontos aleatórios:"
    )
        ...
        with Pool(processes=WORKER_COUNT) as pool:
            ...
            while time.time() < end_time:
                while not results_queue.empty():
                    try:
                        worker_fitness, worker_individual = (
    ...
    print("=" * 60)
    print(
        f"Estratégia Utilizada: Multi-Start Pattern Search ({WORKER_COUNT} execuções paralelas)"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"(Detectado {detected_cpus} processadores lógicos. Usando {WORKER_COUNT}.)"
    )
    if WORKER_COUNT > detected_cpus:
        print(
            f"AVISO: Você pediu {WORKER_COUNT} processos, mas só {detected_cpus} foram detectados."
        )
